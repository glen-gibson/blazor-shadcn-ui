...
- Added early-exit branch before `Convert.ChangeType` (which cannot handle `DateTimeOffset` as it is not `IConvertible`)
...
Added `GlobalSearchValue` parameter with `GlobalSearchValueChanged` callback to enable programmatic control and synchronization of the global search input.
